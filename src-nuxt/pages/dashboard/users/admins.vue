  ...
  <div class="w-full overflow-x-hidden px-4 py-8">
    <div class="flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          <FlapiIcon color="#BDB3FF" :height="20" mode="stroke" name="Shield" viewBox="0 0 24 24" :width="20" />
          <h3 class="text-lg font-semibold text-primary-300">Administrateurs</h3>
        </div>
      </div>
    </div>
...
const users: User[] = usersResponse.data.filter((user: User) => {
  return user.role.name === 'SUPER_ADMIN_FLAPI' || user.role.name === 'ADMIN_CLIENT'
})
